...
void compute_ranks_and_distance(std::vector<Individual> &pop) {
    ...
    for (int m = 0; m < 2; ++m) {
        std::sort(pop.begin(), pop.end(),
                  [m](const Individual &a, const Individual &b) { return a.objectives[m] < b.objectives[m]; });
        pop[0].crowding_dist = pop[n - 1].crowding_dist = 1e10;
        double range = pop[n - 1].objectives[m] - pop[0].objectives[m];
        if (range > 0) {
            for (int i = 1; i < n - 1; ++i)
                pop[i].crowding_dist += (pop[i + 1].objectives[m] - pop[i - 1].objectives[m]) / range;
        }
    }
}

void mutate(Individual &ind, const std::string &name, int n) {
    double mut_prob = 1.0 / n;
    std::uniform_real_distribution<> dis(0, 1);
    for (int i = 0; i < n; ++i) {
        if (dis(gen_rand) < mut_prob) {
            double y = ind.genes[i];
            double yl = (name == "ZDT4" && i > 0) ? -5.0 : 0.0;
            double yu = (name == "ZDT4" && i > 0) ? 5.0 : 1.0;
            double rnd = dis(gen_rand);
            double deltaq = (rnd < 0.5) ? std::pow(2.0 * rnd, 1.0 / (ETA_M + 1.0)) - 1.0
                                        : 1.0 - std::pow(2.0 * (1.0 - rnd), 1.0 / (ETA_M + 1.0));
            ind.genes[i] = std::clamp(y + deltaq * (yu - yl), yl, yu);
        }
    }
}

std::pair<Individual, Individual> crossover(const Individual &p1, const Individual &p2, const std::string &name,
                                            int n) {
    ...
    if (dis(gen_rand) < CROSS_PROB) {
        for (int i = 0; i < n; ++i) {
            if (dis(gen_rand) < 0.5 && std::abs(p1.genes[i] - p2.genes[i]) > 1e-14) {
                ...
                double rnd = dis(gen_rand);
                double betaq = (rnd <= (1.0 / alpha)) ? std::pow(rnd * alpha, 1.0 / (ETA_C + 1.0))
                                                      : std::pow(1.0 / (2.0 - rnd * alpha), 1.0 / (ETA_C + 1.0));
...
void run_experiment(std::string name, int dim) {
    ...
    std::filesystem::path file_path = results_path / (name + "_dim" + std::to_string(dim) + ".csv");
    std::ofstream out(file_path);

    if (!out.is_open()) {
        std::cerr << "Error: Nie mozna otworzyc pliku " << file_path << " do zapisu!" << std::endl;
        return;
    }
    ...
    for (int i = 0; i < POP_SIZE; ++i) {
        Individual ind(dim);
        for (int j = 0; j < dim; ++j) {
            double yl = (name == "ZDT4" && j > 0) ? -5.0 : 0.0;
            double yu = (name == "ZDT4" && j > 0) ? 5.0 : 1.0;
            ind.genes[j] = std::uniform_real_distribution<>(yl, yu)(gen_rand);
        }
        evaluate(ind, name, dim);
        P.push_back(ind);
    }
    ...
    for (int gen = 1; gen <= MAX_GEN; ++gen) {
        ...
        if (gen == 20 || gen == 50 || gen == 100 || gen == 500) {
            for (const auto &ind: P) {
                if (ind.rank == 0)
                    out << gen << "," << ind.objectives[0] << "," << ind.objectives[1] << "\n";
            }
        }
        ...
        while (Q.size() < POP_SIZE) {
            auto select = [&]() {
                int i = std::uniform_int_distribution<>(0, POP_SIZE - 1)(gen_rand);
                int j = std::uniform_int_distribution<>(0, POP_SIZE - 1)(gen_rand);
            ...
            auto [c1, c2] = crossover(P[select()], P[select()], name, dim);
            mutate(c1, name, dim);
            mutate(c2, name, dim);
            evaluate(c1, name, dim);
            evaluate(c2, name, dim);
            Q.push_back(c1);
            Q.push_back(c2);
        }
    ...
    std::cout << "Zapisano: " << file_path.string() << std::endl;
}
